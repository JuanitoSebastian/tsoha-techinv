...
def insert_production(name, starting, ending):
    ...
    result = db.session.execute(
        sql, {"name": name, "starting": starting, "ending": ending})
    production_id = result.fetchone()[0]
    sql = "INSERT INTO userproductions (user_id, production_id) VALUES (:user_id, :production_id)"
    db.session.execute(
        sql, {"user_id": user_id(), "production_id": production_id})
    db.session.commit()


def get_production(production_id):
    sql = "SELECT productions.id, productions.name, productions.starting, productions.ending, users.username creator \
        FROM productions, users, userproductions \
        WHERE productions.id=:production_id AND productions.id=userproductions.production_id AND userproductions.user_id=users.id"
...
def update_production(production_id, name, starting, ending):
    sql = "UPDATE productions SET name = :name, starting = :starting, ending = :ending WHERE id = :production_id"
    db.session.execute(sql, {"name": name, "starting": starting, "ending": ending, "production_id": production_id})
    db.session.commit()
