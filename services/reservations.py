...
def delete_reservation(inventory_id, production_id):
  ...
  db.session.execute(sql, { "inventory_id": inventory_id, "production_id": production_id })
  db.session.commit()

def get_reservations_for_production(production_id):
  sql = "SELECT manufacturers.name manufacturer, equipment.model, inventory.serialnum FROM manufacturers, equipment, inventory, reservations WHERE reservations.production_id=:production_id AND reservations.inventory_id=inventory.id AND equipment.id=inventory.model_id AND manufacturers.id=equipment.manufacturer_id"
